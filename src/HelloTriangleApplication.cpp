...
{
    vkWaitForFences(
        getLogicalDevice().getDeviceRef(), 1, &( getSyncObjects().getInFlightFences()[currentFrame] ), VK_TRUE,
        UINT64_MAX
    );

    uint32_t imageIndex;
    VkResult result = vkAcquireNextImageKHR(
        getLogicalDevice().getDeviceRef(), getSwapChain().getSwapChainRef(), UINT64_MAX,
        getSyncObjects().getImageAvailableSemaphores()[currentFrame], VK_NULL_HANDLE, &imageIndex
    );
...
void HelloTriangleApplication::updateUniformBuffer( uint32_t currentImage )
{
    ...
    ubo.proj = glm::perspective(
        glm::radians( 45.0f ),
        getSwapChain().getSwapChainExtent().width / ( float )getSwapChain().getSwapChainExtent().height, 0.1f, 10.0f
    );
    ubo.proj[1][1] *= -1;
...
void HelloTriangleApplication::createDescriptorSets()
{
    ...
    for( size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++ )
    {
        ...
        vkUpdateDescriptorSets(
            getLogicalDevice().getDeviceRef(), static_cast<uint32_t>( descriptorWrites.size() ),
