...
SyncObjects::SyncObjects()
{
    ...
    {
        if (vkCreateSemaphore(App::get().getLogicalDevice().getDeviceRef(), &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
            vkCreateSemaphore(App::get().getLogicalDevice().getDeviceRef(), &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) != VK_SUCCESS ||
            vkCreateFence(App::get().getLogicalDevice().getDeviceRef(), &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS)
        {
